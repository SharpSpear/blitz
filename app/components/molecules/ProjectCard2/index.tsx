...
import { EditIcon, DeleteIcon } from "app/assets"
...
const ProjectCard = ({
  ...
  const renderActionMenu = ({ direction }) => {
    return (
      <div className="flex justify-center align-center pr-4 absolute">
  ...
  return (
    <>
      <div className={`bg-white rounded-lg shadow-lg mx-auto max-w-[300px] overflow-hidden`}>
        <div className="flex flex-col  items-center">
          <div className="px-5 flex mt-[20px] justify-between w-full">
            <p className="text-[18px] font-Raleway-Bold ">{projectName}</p>
          ...
          {imageUrl ? (
            <>
              {(imageUrl as any).match(
                ...
                <>
                  <VideoPlayer url={imageUrl} autoplay loop />
                </>
              )}
            </>
          ) : (
            <div className="w-[300px] h-[200px] bg-gray-300 flex justify-center items-center">
              <p className="text-5xl text-gray-400">{projectName.charAt(0)}</p>
            </div>
          )}
          <div className="flex w-full mt-[40px] px-3">
            <p style={{ color: "#7A7A7A" }} className="font-Opensans text-[14px] ">{`By ${
              owner?.firstName + " " + owner?.lastName
            }`}</p>
            <p
              style={{ color: "#7A7A7A" }}
              className="font-Opensans text-[14px] ml-auto"
            >{`${numberOfChanges} Record${numberOfChanges === 1 ? "" : "s"}`}</p>
          </div>
        </div>
